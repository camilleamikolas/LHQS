...
class SignalCore():
    ...
    def set_frequency(self, freq, acknowledge=False):
    ...
    def ramp_to_frequency(self, frequency, duration=0.5):
        """ 
        """
    ...
    @set_power.setter
    def set_power(self, pdBm = -10, acknowledge = False):
        if pdBm < -25:
            pdBm = -25
            print('Warning: Available range: -25 to +13 dBm')
...
if __name__ == '__main__':
    sc = SignalCore(name="SignalCore",address="10003053")
    # sc.set_clock_reference(ext_ref=False)
    sc.set_power(-20)
    sc.set_output_state(True)
    # sc.set_rf_mode(0)
    #sc.set_frequency(11370000000)
